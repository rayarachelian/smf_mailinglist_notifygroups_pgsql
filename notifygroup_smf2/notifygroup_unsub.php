...
Upgraded for SMF2 (Dec.31 2011)
*/

if (!defined('SMF'))
	die('Hacking attempt...');
	
function notifygroup_unsub() {
	...
	if (!isset($_POST['email'])||!isset($_POST['set'])) {
		...
		if($smcFunc['db_num_rows']($q)>0) {
			...
			if($a['notify_announcements']!=$context['set']) {
				...
				log_error($txt['notifygroup_prefUpdated']." ".$context['username']." / ".$context['email']." \n\n".($context['set']==0? $txt['notifygroup_setOff']:$txt['notifygroup_setOn']));
				}
		}
		else {
			$context['username']=false;
			log_error($txt['notifygroup_prefUpdated']." ? / ".$context['email']." \n\n".$txt['notifygroup_emailNotExist']);
		}
	}
	
	loadTemplate('notifygroup_unsub');
}
?>